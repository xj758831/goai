#!/usr/bin/env python3
"""S10 lidar capability environment with a bounded short-peak torque envelope.

The official MJCF remains untouched.  Each environment owns an in-memory
``MjModel`` whose actuator and joint force ranges may be widened for a
research-only sensitivity test.  Continuous time above the official 50/14 Nm
limits is measured at every 1 kHz MuJoCo physics step.
"""
...
LEG_CONTINUOUS_LIMIT_NM = 50.0
WHEEL_CONTINUOUS_LIMIT_NM = 14.0
MAX_CONTINUOUS_EXCEEDANCE_S = 0.5
EXPECTED_ACTUATORS = 16


class S10M20LidarTorqueEnvelopeEnv(S10M20LidarCapabilityEnv):
    """Evaluate an S10 policy under an in-memory short-peak torque envelope."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if leg_peak_nm < LEG_CONTINUOUS_LIMIT_NM or wheel_peak_nm < WHEEL_CONTINUOUS_LIMIT_NM:
            raise ValueError("peak limits cannot be below the official 50/14 Nm model limits")
        ...
        if int(np.count_nonzero(self._wheel_mask)) != 4:
            raise RuntimeError(f"expected four S10 wheel actuators, found {np.count_nonzero(self._wheel_mask)}")
    ...
    def _configure_in_memory_limits(self) -> None:
        ...
        actual_joint = np.asarray(
            [np.max(np.abs(self.model.jnt_actfrcrange[joint_id])) for joint_id in self._joint_ids],
            dtype=np.float64,
        )
        if not np.allclose(actual_ctrl, self._peak_limits_nm) or not np.allclose(
            actual_joint, self._peak_limits_nm
        ):
            raise RuntimeError("failed to apply the requested in-memory S10 torque envelope")
    ...
    def _sample_torque_envelope(self) -> None:
        ...
        self._duration_violation |= bool(
            np.any(self._current_exceedance_s > self.max_continuous_exceedance_s + 1.0e-12)
        )

    def _torque_envelope_metrics(self) -> dict[str, Any]:
        per_actuator = {
            name: {
                "continuous_limit_nm": float(self._continuous_limits_nm[index]),
                "peak_limit_nm": float(self._peak_limits_nm[index]),
                "peak_abs_torque_nm": float(self._peak_abs_torque_nm[index]),
                "longest_continuous_exceedance_s": float(self._longest_exceedance_s[index]),
            }
    ...
    def _termination_reason(self, metrics: dict[str, Any]) -> str:
        reason = super()._termination_reason(metrics)
        if reason:
            return reason
...
if __name__ == "__main__":
    print("Environment library; use evaluate_s10_m20_lidar_torque_envelopes.py")
